...
// add caminho para puxar a api WTSS, mudanca futura.
...
async function initializeDatabase() {
  ...
  try {
    ...
    const productsCollection = db.collection("produtos_stac");
    ...
    console.log(
      'Coleção "imagem_satelites" limpa para receber dados atualizados.'
    );
    ...
    for (const collection of collections) {
      try {
        ...
        const detailedBands =
          collectionDetails.properties?.["eo:bands"] ||
        ...
        console.error(
          `Falha ao buscar detalhes para ${collection.id}. Pulando.`
        );
      }
    }
    ...
    if (newProducts.length > 0) {
      ...
      console.log(
        `Sincronização concluída! ${newProducts.length} produtos detalhados foram inseridos.`
      );
    } else {
